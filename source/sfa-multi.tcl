...
# start Excel for summary of all files
        ...
        if {$lenfilelist > 1 && $opt(XLSCSV) == "Excel"} {
          if {[catch {
            ...
            set excelVersion [expr {int([$excel1 Version])}]
            if {$excelVersion < 12} {
              set mf [expr {2**8}]
              set extXLS "xls"
...
# vertical orientation for file name
            ...
            if {!$coverageSTEP} {
              ...
              [$excel1 ActiveWindow] TabRatio [expr 0.5]
            }
...
# generated by
            ...
            $cells1($sum) Item $c 1 "Spreadsheet generated by the $str\STEP File Analyzer (v[getVersion])"
            set anchor [$worksheet1($sum) Range [cellRange $c 1]]
            [$worksheet1($sum) Hyperlinks] Add $anchor [join "http://www.nist.gov/el/msid/infotest/step-file-analyzer.cfm"] [join ""] [join "Link to STEP File Analyzer"]

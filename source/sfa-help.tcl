...
- Plane: blue transparent surface (unbounded planes are with shown with a square surface)
...
with the name of the placeholder.  Optional leader lines and a rectangle for the annotation are
shown with black lines.  To identify which annotation a leader line is associated with, the first
and last points of a leader line have a text label.  Leader line symbols show their type and
position with blue text.  Some implemetations of placeholders might not contain all of the graphic
elements.

Images in PMI placeholders are supported.  To see the placeholder image, you must open the Viewer
html file in a local http server (localhost).  If not, the image will appear as a black rectangle.
...
outputMsg "\nHole Features -------------------------------------------------------------------------------------" blue
outputMsg "The position, orientation, dimensions and tolerances for drilled, counterbore, countersink, and
counterdrill hole features are supported.  Hole features in STEP are not the same as explicitly
modeling holes in part geometry.  Hole features can be used without explicitly modeling holes.
...
counterbore, countersink, and counterdrill.  These surfaces are derived from the hole feature
dimensions and not the part geometry.  Countersink holes are cyan and other holes are green, and
can be switched on and off in the Viewer.  If no hole depth is specified (through hole), then only
a short cylindrical surface with the correct diameter is shown.  Flat and conical hole bottoms are
supported if it is not a through hole.  A text label is shown for each hole.  If the report for
Semantic PMI is not generated, then only the text label is shown.
...
See Help > User Guide (section 4.2.3)"
  ...
  $helpView add command -label "Other Features" -command {
outputMsg "\nOther Features ------------------------------------------------------------------------------------" blue
outputMsg "These Viewer features are not documented in the User Guide or elsewhere in the Help.

1 - Section view clipping planes
...
have the wrong position and orientation.

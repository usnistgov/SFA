...
set schemaLinks(AP209)   "https://www.cax-if.org/documents_cae/AP209_HTML/AP209ed2_mim_lf_v1.46.htm"
...
set dimSizeNames [list \
  "curve length" diameter thickness "spherical diameter" radius "spherical radius" "toroidal minor diameter" "toroidal major diameter" \
  "toroidal minor radius" "toroidal major radius" "toroidal high major diameter" "toroidal low major diameter" "toroidal high major radius" \
  "toroidal low major radius"]

# dimension modifiers (Section 5.3, Table 8)
set dimModNames [list \
  "any cross section" "any part of the feature" "area diameter calculated size" "average rank order size" "circumference diameter calculated size" \
  "common tolerance" "continuous feature" "controlled radius" "free state condition" "least squares association criteria" "local size defined by a sphere" \
  "maximum inscribed association criteria" "maximum rank order size" "median rank order size" "mid range rank order size" \
  "minimum circumscribed association criteria" "minimum rank order size" "range rank order size" "specific fixed cross section" square statistical \
  "two point size" "united feature of size" "volume diameter calculated size"]
...
set tzfNames [list \
  "within a circle" "within a cylinder" "cylindrical or circular" spherical "within a sphere" "between two concentric circles" \
  "between two equidistant curves" "between two coaxial cylinders" "between two equidistant surfaces" "non uniform" \
  "between two parallel circles on a conical surface" "between two parallel circles of the same diameter" \
  "between two equidistant complex lines of two parallel straight lines" "between two non-equidistant complex lines or two non-parallel straight lines" \
  "within a cone" "within a single complex surface" "between two equidistant complex surfaces or two parallel planes" \
  "between two non-equidistant complex surfaces or two non-parallel straight planes" unknown]
...
foreach item [list \
  ...
  "datum feature association to geometric tolerance (6.1)" "datum feature association to dimension (6.5.3)" "all datum targets" "point datum target (6.6.1)" \
  "line datum target (6.6.1)" "rectangle datum target (6.6.1)" "circle datum target (6.6.1)" "circular curve datum target (6.6.1)" \
  "curve datum target (6.6.2)" "area datum target (6.6.2)" "placed datum target geometry (6.6.3)" "movable datum target (6.6.4)" \
...
# dimension modifiers
set section1 "5.3 or 6.9.3 or 6.9.7"
set pmiModifiersArray(any_cross_section,$section1)                    "ACS"
...
set pmiModifiersArray(least_squares_association_criteria,5.3)         "(GG)"
...
# geometric tolerance modifiers
set section2 "6.9.3 or 6.9.7"
set pmiModifiersArray(common_zone,6.9.3)                      "CZ"
set pmiModifiersArray(each_radial_element,6.9.3)              "ERE"
set pmiModifiersArray(free_state,$section2)                   "\u24BB"
set pmiModifiersArray(least_material_requirement,$section2)   "\u24C1"
set pmiModifiersArray(line_element,6.9.3)                     "LE"
set pmiModifiersArray(major_diameter,$section2)               "MD"
set pmiModifiersArray(maximum_material_requirement,$section2) "\u24C2"
set pmiModifiersArray(minor_diameter,$section2)               "LD"
set pmiModifiersArray(not_convex,6.9.3)                       "NC"
set pmiModifiersArray(pitch_diameter,$section2)               "PD"
...
set pmiModifiersArray(associated_minimum_inscribed_feature)    "\u24C3"
set pmiModifiersArray(associated_tangent_feature)              "\u24C9"
set pmiModifiersArray(associated_maximum_inscribed_feature)    "\u24CD"
set pmiModifiersArray(reference_minimum_circumscribed_feature) "N"
set pmiModifiersArray(reference_maximum_inscribed_feature)     "X"
set pmiModifiersArray(total_range_deviations)                  "T"
set pmiModifiersArray(peak_height)                             "P"
set pmiModifiersArray(valley_depth)                            "V"
set pmiModifiersArray(standard_deviation)                      "Q"
set pmiModifiersArray(reference_minimax_feature_without_constraint)                     "C"
set pmiModifiersArray(reference_minimax_feature_with_external_material_constraint)      "CE"
set pmiModifiersArray(reference_minimax_feature_with_internal_material_constraint)      "CI"
set pmiModifiersArray(reference_least_square_feature_without_constraint)                "G"
set pmiModifiersArray(reference_least_square_feature_with_external_material_constraint) "GE"
set pmiModifiersArray(reference_least_square_feature_with_internal_material_constraint) "GI"
...
# not in current recommended practices
...
set pmiModifiersArray(round_hole)                 ""
...
apll_point apll_point_on_curve apll_point_with_surface bounded_curve_with_length connected_edge_sub_set edge_based_topological_representation_with_length_constraint \
edge_bounded_curve_with_length subpath]]
...
symmetric_shape_aspect tangent terminal_feature terminal_location_group transport_feature twisted_cross_sectional_group_shape_element]]
...
vector_style view_volume annotation_placeholder_leader_line annotation_placeholder_occurrence_with_leader_line annotation_to_annotation_leader_line \
...
externally_defined_dimension_definition feature_for_datum_target_relationship flatness_tolerance general_datum_reference geometric_tolerance \
...
runout_zone_definition runout_zone_orientation runout_zone_orientation_reference_direction shape_dimension_representation straightness_tolerance \
surface_profile_tolerance symmetry_tolerance tolerance_value tolerance_zone tolerance_zone_definition tolerance_zone_form tolerance_zone_with_datum \
total_runout_tolerance unequally_disposed_geometric_tolerance]]
...
universal_pair_range universal_pair_value universal_pair_with_range geometry_to_topology_item_association geometry_to_topology_model_association \
topology_to_geometry_item_association topology_to_geometry_model_association]]
...
fillet flat_face gear general_feature hole_bottom hole_in_panel instanced_feature joggle linear_profile locator marking modified_pattern ngon_closed_profile \
open_path_profile outer_round outside_profile partial_circular_profile path_feature_component pattern_offset_membership pattern_omit_membership pocket pocket_bottom \
profile_floor protrusion rectangular_closed_profile rectangular_pattern removal_volume replicate_feature revolved_profile rib rib_top rib_top_floor round_hole \
rounded_end rounded_u_profile simplified_counterbore_hole_definition simplified_counterdrill_hole_definition simplified_countersink_hole_definition \
simplified_spotface_hole_definition slot slot_end spherical_cap spotface_definition spotface_hole_definition square_u_profile step taper tee_profile thread \
thread_runout transition_feature turned_knurl vee_profile basic_round_hole \
basic_round_hole_occurrence basic_round_hole_occurrence_in_assembly counterbore_hole_occurrence counterbore_hole_occurrence_in_assembly \
counterdrill_hole_occurrence counterdrill_hole_occurrence_in_assembly countersink_hole_occurrence countersink_hole_occurrence_in_assembly \
spotface_occurrence spotface_occurrence_in_assembly]]
...
assembly_component assembly_geometric_constraint assembly_group_component assembly_group_component_definition_placement_link \
...
complex_number_literal_polar component_definition component_feature_relationship composite_curve_transition_locator condition conditional_effectivity \
connected_edge_with_length_set_representation connected_volume_set connected_volume_sub_set connectivity_definition_item_relationship constant_function \
contact_feature_definition contact_feature_definition_fit_relationship contact_feature_fit_relationship containing_message \
criterion_report_item_with_number_of_instances criterion_report_item_with_value current_change_element_assignment curve_distance_geometric_constraint \
curve_length_geometric_constraint curve_segment_set curve_smoothness_geometric_constraint curve_style_parameters_representation curve_style_parameters_with_ends \
curve_with_excessive_segments curve_with_small_curvature_radius data_equivalence_assessment_specification data_equivalence_criteria_representation \
data_equivalence_criterion data_equivalence_definition data_equivalence_definition_relationship data_equivalence_definition_representation_relationship \
data_equivalence_inspected_element_pair data_equivalence_inspection_criterion_report data_equivalence_inspection_criterion_report_item \
...
data_equivalence_inspection_result_with_judgement data_equivalence_report_request data_quality_assessment_measurement_association \
data_quality_assessment_specification data_quality_criteria_representation data_quality_criterion data_quality_criterion_assessment_association \
data_quality_criterion_measurement_association data_quality_definition data_quality_definition_relationship data_quality_definition_representation_relationship \
...
different_assembly_volume different_bounding_box different_centroid different_component_identification_via_multi_level_reference different_component_shape \
different_component_type different_curve_length different_length_of_assembly_constraint different_number_of_closed_shell different_number_of_components \
different_number_of_geometric_elements different_number_of_geometric_elements_wireframe_model different_number_of_topological_elements \
...
fixed_instance_attribute_set free_edge free_form_assignment free_form_constraint free_form_relation free_variable_semantics frozen_assignment \
...
gap_between_pcurves_related_to_an_edge gap_between_vertex_and_base_surface gap_between_vertex_and_edge general_linear_function \
...
half_space_2d hierarchical_interface_connection high_degree_axi_symmetric_surface high_degree_conic high_degree_linear_curve \
...
location_representation_role location_role machining_process_executable make_from_feature_relationship mated_part_relationship mathematical_description \
maths_boolean_variable maths_enum_literal maths_function maths_integer_variable maths_real_variable maths_space maths_string_variable maths_tuple_literal \
maths_variable maths_value_precision_qualifier mating_material mating_material_items mechanical_design_requirement_item_association message_contents_assignment \
message_contents_group \
message_relationship mismatch_of_arcwise_connected_curves mismatch_of_arcwise_connected_surfaces mismatch_of_arcwise_connected_surfaces_boundary \
mismatch_of_component mismatch_of_edges mismatch_of_faces mismatch_of_point_cloud_and_related_geometry mismatch_of_points mismatch_of_underlying_edge_geometry \
mismatch_of_underlying_face_geometry missing_assembly_constraint missing_component missing_edge missing_face model_geometric_view modify_element \
multi_level_reference_designator multiply_defined_cartesian_points multiply_defined_curves multiply_defined_directions multiply_defined_edges \
multiply_defined_faces multiply_defined_geometry multiply_defined_placements multiply_defined_solids multiply_defined_surfaces multiply_defined_vertices \
narrow_surface_patch near_point_relationship nearly_degenerate_geometry nearly_degenerate_surface_boundary nearly_degenerate_surface_patch \
neutral_sketch_representation non_agreed_accuracy_parameter_usage non_agreed_scale_usage non_agreed_unit_usage non_manifold_at_edge non_manifold_at_vertex \
non_referenced_coordinate_system non_smooth_geometry_transition_across_edge open_closed_shell open_edge_loop organization_type organization_type_assignment \
organization_type_role oriented_joint over_used_vertex overcomplex_geometry overcomplex_topology_and_geometry_relationship \
...
plane_angle_and_length_pair plane_angle_and_ratio_pair pmi_requirement_item_association pogc_with_dimension point_distance_geometric_constraint \
point_on_edge_curve point_on_face_surface polar_complex_number_region polygonal_area previous_change_element_assignment primitive_2d \
primitive_2d_with_inner_boundary procedural_solid_representation_sequence product_as_planned product_data_and_data_equivalence_relationship \
product_data_and_data_quality_relationship product_definition_formation_resource product_definition_occurrence product_definition_occurrence_reference \
product_definition_occurrence_reference_with_local_representation product_definition_reference product_definition_reference_with_local_representation \
product_definition_relationship_relationship product_definition_specified_occurrence product_definition_usage_relationship product_design_to_individual \
product_design_version_to_individual product_group product_group_attribute_assignment product_group_attribute_set product_group_attributes \
product_group_context product_group_membership product_group_membership_rules product_group_purpose product_group_relationship product_group_rule \
product_group_rule_assignment product_group_rules product_in_attachment_slot product_planned_to_realized product_relationship product_resource quantifier_expression \
radius_geometric_constraint rationalize_function real_interval_from_min real_interval_to_max real_tuple_literal rectangular_area \
rectangular_array_placement_group_component rectangular_composite_surface_transition_locator regular_table_function reindexed_array_function repackaging_function \
repositioned_neutral_sketch representation_proxy_item restriction_function rgc_with_dimension rigid_subsketch same_as_external_item_assignment scan_3d_model \
scan_data_shape_representation scanned_data_item scanner_basic_properties scanner_property sdgc_with_dimension selector_function self_intersecting_curve \
...
shape_feature_definition_element_relationship shape_feature_definition_fit_relationship shape_feature_definition_relationship shape_feature_fit_relationship \
...
small_area_surface_patch small_volume_solid software_for_data_quality_check solid_with_excessive_number_of_voids solid_with_wrong_number_of_voids \
standard_table_function state_definition_to_state_assignment_relationship state_observed state_observed_assignment \
...
steep_angle_between_adjacent_edges steep_angle_between_adjacent_faces steep_geometry_transition_across_edge strict_triangular_matrix structured_message \
su_parameters subsketch summary_report_request summary_report_request_with_number_of_instances surface_distance_assembly_constraint_with_dimension \
surface_distance_geometric_constraint surface_patch_set surface_smoothness_geometric_constraint surface_with_excessive_patches_in_one_direction \
surface_with_small_curvature_radius swept_curve_surface_geometric_constraint swept_point_curve_geometric_constraint symmetric_banded_matrix symmetric_matrix \
symmetry_geometric_constraint system_breakdown_context system_element_usage tangent_assembly_constraint tangent_geometric_constraint texture_style_specification \
texture_style_tessellation_specification thermal_component topology_related_to_multiply_defined_geometry topology_related_to_nearly_degenerate_geometry \
topology_related_to_overlapping_geometry topology_related_to_self_intersecting_geometry transformation_with_derived_angle triangular_matrix \
unbound_parameter_environment unbound_variational_parameter unbound_variational_parameter_semantics uniform_product_space \
...
applied_action_method_assignment applied_action_request_assignment applied_approval_assignment applied_attribute_classification_assignment \
...
back_chaining_rule back_chaining_rule_body beveled_sheet_representation binary_boolean_expression binary_function_call \
...
dimension_curve_terminator_to_projection_curve_associativity directed_action div_expression document document_file document_identifier \
...
extruded_face_solid_with_multiple_draft_angles extruded_face_solid_with_trim_conditions fact_type featured_shape format_function \
...
functionally_defined_transformation generic_expression generic_variable geometric_model_element_relationship global_assignment ground_fact group \
group_assignment group_relationship id_attribute identification_assignment identification_assignment_relationship identification_role \
...
interpolated_configuration_sequence interval_expression iso4217_currency known_source language language_assignment length_function \
...
literal_number local_time log_function log10_function log2_function logical_literal lot_effectivity make_from_usage_option material_designation \
...
minimum_function minus_expression minus_function mod_expression modified_solid modified_solid_with_placed_configuration motion_link_relationship \
...
package_product_concept_feature partial_document_with_structured_text_representation_assignment person \
...
tan_function thickened_face_solid time_interval time_interval_assignment time_interval_based_effectivity time_interval_relationship time_interval_role \

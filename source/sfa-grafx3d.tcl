...
proc x3dFileStart {} {
  ...
  global x3dFile x3dFileName x3dHeight x3dMax x3dMin x3dMsgColor x3dStartFile x3dTitle x3dWidth
...
# x3d title
  ...
  if {[info exists timeStamp] && !$opt(VIZPRTONLY)} {
    if {$timeStamp != ""} {
      set ts [fixTimeStamp $timeStamp]
      append x3dTitle "&nbsp;&nbsp;&nbsp;$ts"
    }
  }
  if {[info exists cadSystem] && !$opt(VIZPRTONLY)} {
    if {$cadSystem != ""} {
...
proc x3dFileEnd {} {
  global ao brepFile brepFileName datumTargetView entCount nistModelURLs nistName nistVersion
  ...
  global x3dIndex x3dMax x3dMin x3dMsg x3dParts x3dShape x3dStartFile x3dTitle x3dWidth
...
# credits
  ...
  set str "\n<p>Generated by the <a href=\"$url\">$ver\STEP File Analyzer and Viewer [getVersion]</a>"
  append str "&nbsp;&nbsp;[clock format [clock seconds] -format "%d %b %G %H:%M"]"
  if {$viz(PRT) && [string first "AP209" $stepAP] == -1} {
...
# background color radio buttons
  ...
  puts $x3dFile "<input type='radio' name='bgcolor' value='1 1 1' onclick='BGcolor(this.value)'/>White<br>"
  puts $x3dFile "<input type='radio' name='bgcolor' value='.8 .8 .8' checked onclick='BGcolor(this.value)'/>Gray<br>"
...
# B-rep part geometry
proc x3dBrepGeom {} {
  global brepFile brepFileName buttons defaultColor localName mytemp opt viz wdir x3dApps x3dBbox x3dMax x3dMin x3dMsg x3dMsgColor x3dParts
...
# run stp2x3d-part.exe
      if {$opt(DEBUGX3D)} {getTiming}
      catch {exec $stp2x3d --input [file nativename $localName] --quality $opt(x3dQuality) --edge $opt(VIZPRTEDGE) --wire $opt(VIZPRTWIRE) --normal 0} errs
      if {$opt(DEBUGX3D)} {getTiming stp2x3d; outputMsg $errs}
...
# write line
                puts $brepFile $line
                if {$opt(DEBUGX3D)} {
                  foreach item {Group Transform} {
                    if {[string first "<$item" $line] != -1} {
                      set c1 [string first "<" $line]
                      outputMsg "[string first "<" $line]$line"
                      incr indent($c1)
                    }
                  }
                }
            ...
            if {$opt(DEBUGX3D)} {foreach idx [lsort -integer [array names indent]] {outputMsg "indent $idx - $indent($idx)"}}
...
# errors running stp2x3d
        ...
        if {$opt(VIZPRTONLY)} {
          errorMsg " No boundary representation part geometry can be processed."
          lappend x3dMsg "<b>Error generating STEP part geometry</b>"
        }
      } else {
        outputMsg $errs red
        errorMsg "Error generating X3D from the STEP boundary representation part geometry\n Try another STEP file viewer.  See Websites > STEP File Viewers\n Use F8 to run the Syntax Checker to check for STEP file errors.  See Help > Syntax Checker"
        outputMsg " "
        lappend x3dMsg "<b>Error generating STEP part geometry</b>"
      }
    } else {
      errorMsg " ERROR: The program (stp2x3d-part.exe) to convert STEP part geometry to X3D was not found in $mytemp"
    }
  } emsg]} {
...
# credits
    ...
    puts $x3dFile "<Transform scale='$tsize1 $tsize1 $tsize1'><Billboard axisOfRotation='0 0 0'><Shape><Text string='\"Visualization generated by the\",\"NIST STEP File Analyzer and Viewer [getVersion]\"'><FontStyle family='\"SANS\"'></FontStyle></Text><Appearance><Material diffuseColor='0 0 0'></Material></Appearance></Shape></Billboard></Transform>"
...
# no file, show message
    } elseif {$opt(VIZPMI) || $opt(VIZTPG) || $opt(VIZFEA) || $opt(VIZPRT)} {
      if {$opt(XLSCSV) == "None"} {errorMsg "There is nothing in the STEP file to View based on the selections on the Options tab."}
      return
    }
  }

...
# x3d title
  ...
  if {[info exists timeStamp] && !$opt(partOnly)} {
    if {$timeStamp != ""} {
      set ts [fixTimeStamp $timeStamp]
      append x3dTitle "&nbsp;&nbsp;&nbsp;$ts"
    }
  }
  if {[info exists cadSystem] && !$opt(partOnly)} {
    if {$cadSystem != ""} {
...
# background color, navigation
  ...
  if {[info exists x3dApps]} {
    if {[llength $x3dApps] == 1 && !$viz(PMI) && !$viz(FEA) && !$viz(TPG)} {
...
# background color radio buttons
  ...
  puts $x3dFile "<input type='radio' name='bgcolor' value='$skyBlue' $bgcheck1 onclick='BGcolor(this.value)'/>Blue<br>"
  puts $x3dFile "<input type='radio' name='bgcolor' value='.8 .8 .8' $bgcheck2 onclick='BGcolor(this.value)'/>Gray<br>"
...
# run stp2x3d-part.exe
      ...
      catch {exec $stp2x3d --input [file nativename $localName] --quality $opt(partQuality) --edge $opt(partEdges) --wire $opt(partSketch) --normal $opt(partNormals)} errs
...
# write line
            ...
            if {$sketch} {outputMsg " Model contains sketch geometry" red}
      ...
      if {$np == 0} {
        ...
        if {$emit == ""} {
          ...
          if {$x3dIndexType == "face"} {
            puts $f "<Shape$defstr><Appearance><Material diffuseColor='$x3dColor' specularColor='0 0 0' shininess='0'></Material></Appearance>"
          } else {

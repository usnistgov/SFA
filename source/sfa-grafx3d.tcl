...
proc x3dFileStart {} {
  ...
  if {!$opt(VIZBRPCLR)} {
    ...
    set x3dFileName $x3dir\-sfa-color.html
  }
  ...
  if {$opt(VIZBRP)} {
    ...
    if {$ok && !$opt(VIZBRPCLR) && ($x3dColorAll > 1 || $x3dColorOverriding > 0)} {
    ...
    if {[info exists x3dApps]} {
      if {[llength $x3dApps] > 1} {
        ...
        foreach n [lsort -integer $x3dApps] {
          puts $x3dFile "<nobr><input type='checkbox' checked onclick='togColor$n\(this.value)'/>$x3dAppColor($n)$n </nobr>"
        }
        if {$x3dColorOverriding > 1} {puts $x3dFile "<nobr>($x3dColorOverriding) Overriding colors</nobr>"}
      }
    }
    puts $x3dFile "<p>"
  }
...
# function for BRP, EDG, colors
  if {$viz(BRP)} {
    x3dSwitchScript BRP
    if {$opt(VIZBRPEDG) && $opt(VIZBRPCLR)} {x3dSwitchScript EDG}
    if {[info exists x3dApps]} {
      if {[llength $x3dApps] > 1} {
...
proc x3dBrepGeomColor {} {
  global brepFile brepFileName buttons developer entCount localName mydocs mytemp opt viz wdir x3dAppColor x3dColorAll x3dApps x3dMax x3dMin x3dMsg
...
# start processing
      ...
      #if {$opt(DEBUGX3D)} {getTiming processing}
...
# or from list of shp*.x3d files in the directory, usually when there was an error running stp2x3d-color.exe
      ...
      if {$opt(DEBUGX3D)} {getTiming get-files}
...
# do not check rgb color '0.5 0.5 0.5' in certain cases
      set checkColor 1
      if {![info exists entCount(presentation_style_assignment)] || [llength $flist] == 1 || $x3dColorAll == 0} {set checkColor 0}
...
# process all shp*.x3d files
      foreach fx3d $flist {
        if {[catch {
          set ncoord 0
          incr nfile
          if {[expr {$nfile%500}] == 0} {outputMsg "  $nfile"; update}
...
# report some counters
      outputMsg "  $nfile part X3D files processed"
      set msg "  [llength [array names w]] colors"
      if {$ngray > 0 && $developer} {append msg " ($ngray gray)"}
      outputMsg $msg
      if {$nlineset > 0} {
        outputMsg "  $nlineset edges"
        puts $brepFile "</Group></Switch>"
      }
      if {$opt(DEBUGX3D)} {getTiming done-files}

# set global min-max
      foreach id1 {0 1 2} id2 {x y z} {
        if {$min($id1) !=  1.e10} {set x3dMin($id2) $min($id1)}
        if {$max($id1) != -1.e10} {set x3dMax($id2) $max($id1)}
      }
...
# end b-rep file
      ...
      set viz(BRP) 1
      if {$opt(DEBUGX3D)} {getTiming done-combining}

# delete temporary files
      if {!$opt(DEBUGX3D)} {catch {file delete -force -- $stpx3dDir}}

# errors running stp2x3d-color
      if {[string first "Done!" $errs] == -1} {
        outputMsg "\nERRORS from stp2x3d-color.exe\n$errs" red

# permission errors writing x3d file
        if {[string first "PermissionError" $errs] != -1} {
          set msg " ERROR writing X3D generated by stp2x3d-color.exe to $stpx3dDir"
...
# other errors
          ...
          set msg " ERROR generating some X3D from the STEP part geometry\n"
          if {!$opt(VIZBRPCLR)} {
            append msg "  Try another STEP file viewer.  See Websites > STEP File Viewers"
          } else {
            append msg "  Turn off Color for Part Geometry and try again."
          }
          errorMsg $msg
          outputMsg " "
...
# program not found, try non-color viewer
    } else {
...
# B-rep part geometry
proc x3dBrepGeom {} {
  global brepFile brepFileName buttons defaultColor localName mydocs mytemp opt viz wdir x3dApps x3dColorBrep x3dMax x3dMin x3dMsg
...
# output .x3d file name, account for extra '.' characters because stp2x3d creates the x3d file name based on the position of the first '.'
...
# lists and number of all and overriding colors
  set colorsAll [lrmdups [concat $colors(manifold_solid_brep) $colors(shell_based_surface_model) $colors(advanced_face)]]
  set colorsOverriding [lrmdups $overridingColors]
  set x3dColorAll [llength $colorsAll]
  set x3dColorOverriding [llength $colorsOverriding]
  if {$opt(DEBUGX3D)} {outputMsg [compareLists "colors" $colorsAll $colorsOverriding]}

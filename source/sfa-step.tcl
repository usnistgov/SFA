proc pmiFormatColumns {str} {
  global cells col gpmiRow opt pmiStartCol recPracNames row spmiRow stepAP thisEntType worksheet
...
#-------------------------------------------------------------------------------
# run syntax checker with the command-line version (sfa-cl.exe) and outputing filtered result
...
# output to log file
        if {$opt(LOGFILE)} {
          ...
          set lf [open $lfile w]
          puts $lf "Syntax Checker results for: $fileName\nGenerated by the NIST STEP File Analyzer and Viewer v[getVersion] ([string trim [clock format [clock seconds]]])\n"
          puts $lf $sfaerr
          close $lf

...
# dimensional size, location
  ...
  set dim_size_wdf1 [list composite_unit_shape_aspect_and_dimensional_size_with_datum_feature_and_symmetric_shape_aspect applies_to name]
...
# only skip when an entity refers to itself, very unusual case with dimensional_*_and_datum_feature
    ...
    if {[info exists lastEnt]} {
      if {$lastEnt == "$objID $objType"} {
        errorMsg "  Attribute '$objType.$lastAttr' refers to itself. ($recPracNames(pmi242), Sec. 6.5, Figure 33)" red
        set noskip 0
      }
    }

...
# node, element checkbox script
                    ...
                    #puts $x3domFile "<script>function togND(choice){document.getElementById('swND').setAttribute('whichChoice', choice);}</script>"
...
# transparency script
                    ...
                    puts $x3domFile "\n<FONT FACE=\"Arial\"><H3>Analysis Model for:  [file tail $localName]</H3><UL>"
                    puts $x3domFile "<LI>Visualization of AP209 file generated by the <a href=\"$url\">$str\STEP File Analyzer (v[getVersion])</A> and rendered with <A HREF=\"http://www.x3dom.org/\">X3DOM</A>."
...
proc x3domScript {type} {
  ...
  puts $x3domFile "if (!document.getElementById('sw$type').checked) {document.getElementById('sw$type').setAttribute('whichChoice', -1);} else {document.getElementById('sw$type').setAttribute('whichChoice', 0);}"
  puts $x3domFile "document.getElementById('sw$type').checked = !document.getElementById('sw$type').checked;\n}</script>"
  #puts $x3domFile "<script>function tog$type\(choice){document.getElementById('sw$type').setAttribute('whichChoice', choice);}</script>"
}